...
public:
    ...
    Obfuscation(const std::array<std::byte, SIZE_BYTES>& key_arr) : Obfuscation(Span(key_arr)) {}
    Obfuscation(const std::vector<unsigned char>& vector) : Obfuscation(MakeByteSpan(vector)) {}

    uint64_t Key() const { return rotations[0]; }
    bool IsActive() const { return Key() != 0; }
...
template <typename Stream> void Serialize(Stream& s, const Obfuscation& a) { ser_writedata64(s, a.Key()); }
template <typename Stream> void Unserialize(Stream& s, Obfuscation& a) { a = ser_readdata64(s); }

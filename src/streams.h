...
namespace util {
template<typename T>
inline void XorAligned(Span<std::byte> write, const T key, const size_t n)
{
    assert(n <= write.size());
    T raw;
...
inline void Xor(Span<std::byte> write, const uint64_t key)
{
    ...
    for (; write.size() >= 8; write = write.subspan(8)) {
        XorAligned<uint64_t>(write, key, 8);
    }
    switch (write.size()) {
    case 0: return;
    case 1: XorAligned<uint8_t>(write, key, 1); return;
    case 2: XorAligned<uint16_t>(write, key, 2); return;
    case 4: XorAligned<uint32_t>(write, key, 4); return;
    default: XorAligned<uint64_t>(write, key, write.size());
    }
}

...
class UTXOCacheTracepointTest(BitcoinTestFramework):
    ...
    def test_flush(self):
        ...
        UTXOS_IN_CACHE = 3  # Number of UTXOs in cache because scantxoutset doesn't wipe anymore
        expected_flushes.append({"mode": "FORCE_SYNC", "for_prune": False, "size": UTXOS_IN_CACHE})
        expected_flushes.append({"mode": "FORCE_SYNC", "for_prune": False, "size": 0})
        self.stop_node(0)
